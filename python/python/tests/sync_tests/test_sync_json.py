...
from glide.commands.sync_commands import sync_glide_json as json
...
from glide.sync.glide_client import GlideClusterClient, TGlideClient
...
class TestSyncJson:
    ...
    def test_sync_json_get_formatting(self, glide_sync_client: TGlideClient):
        ...
        result = json.get(
            glide_sync_client, key, "$", JsonGetOptions(indent="  ", newline="\n", space=" ")
        )
    ...
    def test_sync_json_strappend(self, glide_sync_client: TGlideClient):
        ...
        with pytest.raises(RequestError):
            json.strappend(
                glide_sync_client, "non_exiting_key", OuterJson.dumps("try")
            )
    ...
    def test_sync_json_arrindex(self, glide_sync_client: TGlideClient):
        ...
        result = json.arrindex(
            glide_sync_client, key, "$.nested_structure.level1.level2.level3[*]", '"gamma"'
        )
        assert result == [
            0,
            2,
    ...
    def test_sync_json_arrpop(self, glide_sync_client: TGlideClient):
        ...
        assert (
            json.set(glide_sync_client, key2, "$", '[[], ["a"], ["a", "b", "c"]]')
            == OK
        )
        assert (
        ...
        assert json.arrpop(glide_sync_client, key2, JsonArrPopOptions("$[0]")) == [
            None
        ]
    ...
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_json_batch_array(self, glide_sync_client: GlideClusterClient):
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_json_batch(self, glide_sync_client: GlideClusterClient):
        transaction = ClusterTransaction()
